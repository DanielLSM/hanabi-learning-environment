...
Transition = collections.namedtuple(
    'Transition',
    ['reward', 'observation', 'legal_actions', 'action', 'begin'])
...
class DQNAgent(object):
    ...
    """
        ...
        sync_qt_ops = []
        trainables_online = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                              scope='Online')
        trainables_target = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                              scope='Target')
    ...
    """
        ...
        self._record_transition(current_player,
                                0,
                                observation,
                                legal_actions,
                                self.action,
    ...
    """
        ...
        self.action = self._select_action(observation, legal_actions)
        self._record_transition(current_player, reward, observation,
    ...
    """
        self.transitions[current_player].append(
            Transition(reward, np.array(observation, dtype=np.uint8,
                                        copy=True),
    ...
    """
        ...
        for player in range(self.num_players):
            ...
            for index, transition in enumerate(self.transitions[player]):
                ...
                self._store_transition(transition.observation,
