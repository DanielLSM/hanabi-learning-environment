...
def initialize_checkpointing(agent,
                             experiment_logger,
                             checkpoint_dir,
  ...
  """
    ...
    if latest_checkpoint_version >= 0:
        ...
        if agent.unbundle(checkpoint_dir, latest_checkpoint_version,
                          dqn_dictionary):
...
def run_one_phase(agent, environment, obs_stacker, min_steps, statistics,
  ...
  """
    ...
    while step_count < min_steps:
        episode_length, episode_return = run_one_episode(
...
def run_one_iteration(agent,
  ...
  """
    ...
    number_steps, sum_returns, num_episodes = (run_one_phase(
        agent, environment, obs_stacker, training_steps, statistics, 'train'))
    ...
    if evaluate_every_n is not None and iteration % evaluate_every_n == 0:
        ...
        for _ in range(num_evaluation_games):
            episode_data.append(
                run_one_episode(agent, environment, obs_stacker))

        eval_episode_length, eval_episode_return = map(np.mean,
                                                       zip(*episode_data))
...
def log_experiment(experiment_logger,
                   iteration,
                   statistics,
...
def checkpoint_experiment(experiment_checkpointer, agent, experiment_logger,
  ...
  """
    if iteration % checkpoint_every_n == 0:
        agent_dictionary = agent.bundle_and_checkpoint(checkpoint_dir,
                                                       iteration)
        if agent_dictionary:
            agent_dictionary['current_iteration'] = iteration
            agent_dictionary['logs'] = experiment_logger.data
            experiment_checkpointer.save_checkpoint(iteration,
                                                    agent_dictionary)
...
def run_experiment(agent,
    ...
    for iteration in range(start_iteration, num_iterations):
        iteration_func(agent, environment, obs_stacker, iteration,
                       training_steps, experiment_logger, logging_file_prefix,
                       log_every_n, experiment_checkpointer, checkpoint_dir,
                       checkpoint_every_n)


def iteration_func(agent, environment, obs_stacker, iteration, training_steps,
                   experiment_logger, logging_file_prefix, log_every_n,
                   experiment_checkpointer, checkpoint_dir,
                   checkpoint_every_n):
    start_time = time.time()
